...
from apmplus_server.sdk.decorators.task import handle_exception
from apmplus_server.instrumentation.ark_runtime.utils import (
    ...
    should_send_prompts,
    model_as_dict,
    _get_ark_runtime_base_url,
    ARK_LLM_USAGE_TOKEN_TYPES,
)
from apmplus_server.instrumentation.ark_runtime.wrappers import (
    _with_chat_telemetry_wrapper,
)
from apmplus_server.sdk.semconv import SpanAttributes, LLMRequestTypeValues
from apmplus_server.sdk.tracing.tracing import set_common_trace_attributes

SPAN_NAME = "ark_runtime.chat"
...
def chat_wrapper(
        ...
        streaming_time_per_output_token: Histogram,
        wrapped,
        instance,
        args,
        kwargs,
    ...
    try:
        ...
        if duration > 0 and duration_histogram:
            duration_histogram.record(duration * 1000, attributes=attributes)
...
def _set_chat_metrics(
    ...
    shared_attributes = {
        ...
        "server.address": _get_ark_runtime_base_url(instance),
    }
    ...
    if duration and isinstance(duration, (float, int)) and duration_histogram:
        duration_histogram.record(duration * 1000, attributes=shared_attributes)
...
def _set_token_counter_metrics(token_counter, usage, shared_attributes):
    for name, val in usage.items():
        if name in ARK_LLM_USAGE_TOKEN_TYPES:
...
def _build_from_streaming_response(
    ...
    complete_response = {"choices": [], "model": "", "usage": None}
    ...
    for item in response:
        ...
        if first_token:
            ...
            shared_attributes["llm.response.model"] = complete_response.get("model")
            span.set_attribute(SpanAttributes.LLM_RESPONSE_TTFT, (time_of_first_token - start_time) * 1000)
            if streaming_time_to_first_token:
                streaming_time_to_first_token.record((time_of_first_token - start_time) * 1000,
    ...
    shared_attributes["llm.response.model"] = complete_response.get("model")
    # choice metrics
    logging.info("streaming complete response: ", json.dumps(complete_response))
    ...
    if usage and token_counter:
        _set_token_counter_metrics(
            token_counter, usage, shared_attributes
        )
    ...
    if streaming_time_to_generate:
        streaming_time_to_generate.record((time.time() - time_of_first_token) * 1000, attributes=shared_attributes)

    if usage and  usage.get("completion_tokens") and streaming_time_per_output_token:
        completion_tokens = usage.get("completion_tokens")
        streaming_time_per_output_token.record((time.time() - time_of_first_token) * 1000 / completion_tokens, attributes=shared_attributes)
        span.set_attribute(SpanAttributes.LLM_RESPONSE_TPOT, (time.time() - time_of_first_token) * 1000 / completion_tokens)
