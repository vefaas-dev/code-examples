# Python MCP Server (veFaaS Template)

This repository provides a Python MCP Server template that can be directly deployed to veFaaS. Recommended workflow:
...
- Ensure `requirements.txt` contains all dependencies (including possible native extension packages like `rpds-py`, `pydantic-core`, etc.)
- Generate deployment package: `./zip.sh`
- Upload the deployment package to the veFaaS console
- Click "Install Dependencies" in the console. The platform will install the dependencies in the Linux Python 3.12 environment to the runtime directory
