...
def opensource_get_commit_log(repo_folder_path):
    ...
    subprocess.run(
        "git log -p > ./opensource_git_commit.log", shell=True, stdout=subprocess.PIPE
    )
...
def check_sensitive_information(
    ...
    for target in target_dirs:
        dir_path = os.path.join(repo_folder_path, target)
        if not os.path.exists(dir_path):
            continue  # skip missing dirs
        for root, dirs, files in os.walk(dir_path):
            dirs[:] = [d for d in dirs if d != "node_modules"]  # prune
            if "node_modules" in root or "site-packages" in root:
            ...
            for file in files:
                ...
                ) or (os.path.join(root, file).find("/.git/") > -1):
                    continue
                with open(os.path.join(root, file), "r", encoding="ISO-8859-1") as f:
                    for line in f.readlines():
                        cnt += 1
                        if (
                            should_ignore_keywords(ignore_list_keywords, line.strip())
                            == False
                        ):
                            for pattern in keywords_list:
                                ...
                                if (
                                    pattern
                                    == "(tokenizer|transformer|token_id|tokenid|attention_head).{0,20}"
                                ):
                ...
                with open(os.path.join(root, file), "r", encoding="ISO-8859-1") as g:
                    ...
                    if str(file).endswith(".json"):
                        ...
                        # 检查每组关键词是否都在文件内容中
                        if all(
                            keyword in file_content for keyword in aigc_keywords_group1
                        ):
                            print(
                                '\033[1;36mFile "'
                                + os.path.join(root, file)
                                + " have some aigc sensitive information: "
                                + "\033[1;32m"
                                + str(aigc_keywords_group1)
                                + "\033[0m"
                            )
                            with open(
                                "./sensitive_info_result.txt", "a+", encoding="utf-8"
                            ) as g:
                                g.write(
                                    os.path.join(root, file)
                                    + " have some aigc sensitive information: "
                            ...
                            keyword in file_content for keyword in aigc_keywords_group2
                        ):
                            print(
                                '\033[1;36mFile "'
                                + os.path.join(root, file)
...
def main():
    cnt = 0
    repo_folder_path = "./"  # 不区分操作系统，填写项目路径即可
